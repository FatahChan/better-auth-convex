...
installed component.
  ...
  <div className="fd-step">
    ...
    // Typed wrapper for app-side adapter calls (see Usage below)
...
#### Querying with the typed adapter

Don't call `ctx.runQuery(components.betterAuth.adapter.findOne, …)` directly
from app code — return types won't narrow by `model`. Use
`authComponent.typedAdapter` instead (requires your local schema in
`createClient` config):
...
See [`typedAdapter`](/api/type-utilities#typedadapter-on-the-component-client) for
setup and [`createTypedAdapter`](/api/type-utilities#createtypedadapter) for the
lower-level API.
